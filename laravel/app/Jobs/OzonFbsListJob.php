...
use App\Models\OzonPostingFbs;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Carbon;

class OzonFbsListJob implements ShouldQueue
{
    ...
    {
        $response = $this->ozon->fbs_list($this->date_from, $this->date_to);
        if($response['result']) {
            $count = 0;
            foreach($response['data']['result']['postings'] as $item) { 
                if(isset($item['products'])) {
                    foreach($item['products'] as $index => $product) {
                        $ozon_item = new OzonPostingFbs();
                        ...
                        $ozon_item->delivery_method_id = $item['delivery_method']['id'];   
                        $ozon_item->delivery_method_name = $item['delivery_method']['name'];   
                        $ozon_item->warehouse_id = $item['delivery_method']['warehouse_id'];   
                        $ozon_item->warehouse = $item['delivery_method']['warehouse'];   
                        $ozon_item->tpl_provider_id = $item['delivery_method']['tpl_provider_id'];   
                        $ozon_item->tpl_provider = $item['delivery_method']['tpl_provider'];   
                            
                        $ozon_item->tracking_number = $item['tracking_number'];   
                        $ozon_item->tpl_integration_type = $item['tpl_integration_type'];   
                        $ozon_item->in_process_at = new Carbon($item['in_process_at']);                                 
                        $ozon_item->shipment_date = new Carbon($item['shipment_date']);   
                        $ozon_item->delivering_date = new Carbon($item['delivering_date']);  
                        
                        $ozon_item->cancel_reason_id = $item['cancellation']['cancel_reason_id'];   
                        $ozon_item->cancel_reason = $item['cancellation']['cancel_reason'];   
                        $ozon_item->cancellation_type = $item['cancellation']['cancellation_type'];   
                        $ozon_item->cancelled_after_ship = $item['cancellation']['cancelled_after_ship'];   
                        $ozon_item->affect_cancellation_rating = $item['cancellation']['affect_cancellation_rating'];   
                        $ozon_item->cancellation_initiator = $item['cancellation']['cancellation_initiator'];   

                        $ozon_item->customer = $item['customer'];                              
                        ...
                        $ozon_item->mandatory_mark = $product['mandatory_mark'];   

                        $ozon_item->addressee = $item['addressee'];    
                        $ozon_item->barcodes = $item['barcodes'];    
                        ...
                        $ozon_item->delivery_date_begin = new Carbon($item['analytics_data']['delivery_date_begin']);
                        $ozon_item->delivery_date_end = new Carbon($item['analytics_data']['delivery_date_end']);
                        ...
                        $ozon_item->is_express = $item['is_express']; 
                        $ozon_item->products_requiring_gtd = $item['requirements']['products_requiring_gtd']; 
                        $ozon_item->products_requiring_country = $item['requirements']['products_requiring_country']; 
                        $ozon_item->products_requiring_mandatory_mark = $item['requirements']['products_requiring_mandatory_mark']; 
                        $ozon_item->products_requiring_rnpt = $item['requirements']['products_requiring_rnpt']; 

                        $ozon_item->parent_posting_number = $item['parent_posting_number'];  
                        $ozon_item->multi_box_qty = $item['multi_box_qty'];  
                        $ozon_item->is_multibox = $item['is_multibox'];  
            ...
            Log::info('[OZON] Список отправлений ФБС успешно загружен! Кол-во записей: ' . $count);
        }
    }
}
